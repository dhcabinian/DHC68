...
class GUI(object):
    ...
    def creategui(self):
        ...
        #David Added Underglow command
        menu_view.add_command(label='RGB Underglow Configuration', command=self.showrgb_underglow)
    ...
    def showrgb(self):
        if self.selectedconfig:
            ...
            messagebox.showerror(title="Can't Configure LEDs",
                                 ...
                                 parent=self.root)
    #added by David
    def showrgb_underglow(self):
        if self.selectedconfig:
            try:
                if configurations[self.selectedconfig].use_rgb_underglow:
                    RGBWindowUnderglow(self.root, self, "RGB Underglow Configuration", self.rgb_underglow)
                else:
                    raise AttributeError
            except AttributeError:
                messagebox.showerror(title="Can't Configure RGB Underglow",
                                 message="This keyboard doesn't currently have support for RGB Underglow.",
                                 parent=self.root)
        else:
            messagebox.showerror(title="Can't Configure LEDs",
                                 message='Create a keyboard first!',
    ...
    def savefile(self):
        if self.selectedconfig:
            ...
            package = (SAVE_VERSION, self.selectedconfig, self.maps,
                       ...
                       self.ledlayers, self.rgb, self.rgb_underglow)
                       #David addded rgb_underglow
    ...
    def overlay(self, hexdata):
        ...
        #David added new try catch for underglow
        try:
            if config.use_rgb_underglow:
                address = config.firmware.rgb_underglow_pin
                offset = address - start
                bytes[offset] = config.rgb_underglow_pin

                address = config.firmware.rgb_underglow_count
                offset = address - start
                bytes[offset] = config.rgb_underglow_count

                address = config.firmware.rgb_underglow_map
                offset = address - start
                for n in range(len(self.rgb_underglow)):
                    bytes[offset+n] = self.rgb_underglow[n]
            else:
                raise AttributeError
        except AttributeError:
            print("Test")
...
#David Added RGB WIndow Underglow Class
class RGBWindowUnderglow(simpledialog.Dialog):
    ...
    def callback(self, event):
        webbrowser.open_new(self.config.rgb_underglow_url)
    ...
    def colorparse(self, string, order):
        ...
        for y in order:
            ...
            b = int(x[4:6], 16)
            total = r+g+b+1
            if (total > self.config.rgb_underglow_max):
                        r = int(r * self.config.rgb_underglow_max/total)
                        g = int(g * self.config.rgb_underglow_max/total)
                        b = int(b * self.config.rgb_underglow_max/total)
            rgb_underglow.extend([g,r,b])
        return rgb_underglow

    def apply(self):
        self.gui.rgb_underglow = self.colorparse(eval('"""' + self.t.get(1.0, END) + '"""'), self.config.rgb_underglow_order)
